...
function handle(userInfo, payload, next) {
  ...
  handles[userInfo.state](userInfo, payload, next);
}
...
handles[States.NO_STATE] = function(userInfo, payload, next) {
  userInfo.state = States.NEW_USER;
  User.save(userInfo, function(err) {
    if (err) return next(err);
    handle(userInfo, payload, next);
  });
}

handles[States.NEW_USER] = function(userInfo, payload, next) {
  twilio.sendMessage({
    to: userInfo.number,
    from: conf.get('twilioNumber'),
    body: ''
...
handles[States.WAITING_FOR_VERIFICATION] = function(userInfo, payload, next) {
  if (!payload) {
    return next(new Error('No input received for state '
      + States.WAITING_FOR_VERIFICATION));
  }

  var normalized = payload.Body.trim().toLowerCase();
...
handles[States.VERIFIED] = function(userInfo, payload, next) {
  twilio.sendMessage({
    to: userInfo.number,
    from: conf.get('twilioNumber'),
    body: ''
...
handles[States.READY_FOR_PROMPT] = function(userInfo, payload, next) {
...
handles[States.PROMPT_QUEUED] = function(userInfo, payload, next) {
  if (payload) return next(new Error('How nice! But I haven\'t asked you anything yet.'));
  else return next(null);
}

handles[States.WAITING_FOR_RESPONSE] = function(userInfo, payload, next) {
  if (!payload) return next(new Error('I missed your response. Try again?'));

  var media = [];
  for (var i = 0; i < parseInt(payload.NumMedia, 10); i++) {
    media.push(payload['MediaUrl' + i]);
  }

  Prompt.answer(userInfo.hash, {body: payload.Body, media: media}, function(err) {
    if (err) return next(err);
    userInfo.state = States.READY_FOR_PROMPT;
    User.save(userInfo, function(err) {
      if (err) return next(err);
